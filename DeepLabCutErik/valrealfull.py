...
def pairwisedistances(DataCombined, scorer1, scorer2, pcutoff=-1, bodyparts=None, root_dir='', shuffle_num=None, iteration_num=None):
    ...
    if bodyparts == None:
        ...
        # print(DataCombined[scorer1][bodyparts])
        # print(DataCombined[scorer2][bodyparts])
        # print(type(DataCombined[scorer1][bodyparts]))
        DataCombined[scorer1][bodyparts].to_csv(os.path.join(root_dir,f'{shuffle_num}_{iteration_num}_gt.csv'))
        DataCombined[scorer1][bodyparts].to_csv(os.path.join(root_dir,f'{shuffle_num}_{iteration_num}_pred.csv'))
        return RMSE, RMSE[mask]
...
if __name__ == '__main__':
    ...
    root_dir = os.path.join('D:\Chenqi\KP Detection\DeepLabCutErik', args.run_dir+args.suffix)
    shuffle_result = {}
    ...
    total_error = 0
    desired_pair = []
    print(args.run_dir+args.suffix)
    ...
    root_dir = 'D:\\Chenqi\\KP Detection\\DeepLabCutErik'
    ...
    num_shuffle = len(os.listdir(os.path.join(root_dir, args.run_dir, 'dlc-models', 'iteration-0')))
    ...
    for trainFraction in TrainingFractions:
        ...
        for pred_anno_file in pred_anno_files:
            ...
            RMSE, RMSE_masked = pairwisedistances(
                ...
                cfg["pcutoff"],
                comparisonbodyparts,
                os.path.join(run_dir_real,'evaluation-results','iteration-0'),
                shuffle_num,
                iteration_num)

...
def evaluate(model, dataloader, data, epoch, criterion, split):
    ...
    # calculate the number of batches
    num_batches = int(len(data)/dataloader.batch_size)
    all_outputs = np.empty((0,1))
    all_keypoints = np.empty((0,1))
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader), total=num_batches):
            counter += 1
            image, keypoints = data['image'].to(DEVICE), data['keypoints'].to(DEVICE)
            ...
            ### New version
            loss = MAE_loss(outputs, keypoints)
            running_loss += loss
            all_outputs = np.vstack([all_outputs,outputs.cpu().detach().numpy().reshape((-1,1))])
            all_keypoints = np.vstack([all_keypoints,keypoints.cpu().detach().numpy().reshape((-1,1))])
            
    RMSE_loss(all_outputs,all_keypoints)
...
def MAE_loss(pred, gt):
    ...
    diff = pred-gt
    sq_diff = diff**2
    sq_dist = torch.sum(sq_diff, axis=1)
    mae = torch.sum(torch.sqrt(sq_dist))/pred.size()[0]
    return mae
...
def RMSE_loss(pred, gt):
    ...
    # mae = torch.sum(torch.sqrt(sq_dist))/pred.size()[0]
    print('Mean, S.D.',np.nanmean(np.sqrt(sq_dist)),np.nanstd(np.sqrt(sq_dist)))

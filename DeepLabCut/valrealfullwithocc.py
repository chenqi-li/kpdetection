...
if __name__ == '__main__':
    ...
    for trainFraction in TrainingFractions:
        ...
        for pred_anno_file in pred_anno_files:
            ...
            # print(Data.iloc[0,:])
            # print(DataMachine.iloc[0,2])
            datarow, datacol = np.where(Data.isna())
            for idx in range(datarow.shape[0]):
                if datacol[idx]%2==0:
                    colidx = int(datacol[idx]/2)
                    rowidx = int(datarow[idx])
                    # print(colidx,rowidx)
                    if DataMachine.iloc[rowidx].iloc[colidx*3+2]<0.6:
                        # print(DataMachine.iloc[rowidx].iloc[colidx])
                        occ_correct.append(1)
                        # print(Data, DataMachine, rowidx, colidx)
                    else:
                        occ_correct.append(0)
            # print(DataMachine)
    ...
    print('Percent of Occluded Keypoint Detected', np.sum(occ_correct)/len(occ_correct)*100, '%', 'Number Detected:', np.sum(occ_correct), 'Number Total:', len(occ_correct))
    print('\n')
